...
function StudentBoard({ id, url, name, classname, gender, pointText, ratioText, point, ratio, isEdit, setIsEdit }) {
    ...
    return (
        <div className='flex md:flex-col gap-4 md:gap-0 items-center space-y-3 w-full rounded-lg shadow-lg md:rounded-none md:shadow-none md:w-5/6 h-full mx-auto shadow-style p-6 bg-white'>
            ...
            <div className='w-1/2 md:w-full flex flex-wrap'>
                {isEdit ? (
                    <>
            ...
            {isEdit ? (
                ...
                <div className="cursor-pointer mt-2 px-4 py-[3px] w-full flex justify-center items-center bg-blue-200 rounded-full" onClick={() => setIsEdit(true)}>Chỉnh sửa</div>
            )}
